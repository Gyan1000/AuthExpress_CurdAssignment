...
export const registerUser=async(req,res,next)=>{
    ...
    try{
       const registeredUser=await userModel.findOne({username})
        if(registeredUser)
          return next(new CustomError('User already Exist',504))
         //  const newUser= await userModel.create({...req.body,profile_image:{
         //    public_id: email,
         //    secure_url: 'https://res.cloudinary.com/du9jzqlpt/image/upload/v1674647316/avatar_drzgxv.jpg'
         //   }});

           const newUser= await userModel.create({name,username,email,password,bio,
            avatar:{
            public_id: email,
            secure_url: 'https://res.cloudinary.com/du9jzqlpt/image/upload/v1674647316/avatar_drzgxv.jpg'}
          });
        ...
        if (req.file) {
          ...
          try {
           ...
           res.status(200).send({msg:"user registered successfully",success:true,data:newUser});
   ...
   try{
      ...
      if(userData)
      {
         if(await bcrypt.compare(password,userData.password))
         {
            ...
            res.status(200).send({msg:"USER LOGIN SUCCESSFULLY ,ENJOY!",success:true,data:userData});
         }
         else
         {
...
//    crossOriginIsolated.log("<<<<<<<<<<<>>>>>>>>>>",userInfo)
...
// }
   ...
   try {
     const authenticUser = await userModel.findById(userInfo.id).select('-password');
     res.status(201).send({
       message: 'Authentic User',
       success: true,
       data: authenticUser,
